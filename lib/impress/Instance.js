const
  ...
  DEFAULT_COUNT_TO_RESTART = 3,
  PHANTOM_TTL_SPREAD_FACTOR = .2,
  TIMEOUT_APPENDIX = 100,
  MIN_PHANTOM_RESTART_INTERVAL = 1000
  ;
...
function Instance(portPool, htmlFilters, options) {
  ...
  this.countToRestart = options.countToRestart || DEFAULT_COUNT_TO_RESTART;
...
Instance.prototype = {
  ...
  _phantomRestart: function() {
    ...
    function run() {
      self._cancelPhantomRestartDelay();
      ...
      self._phantom.closePromise(function() {
        logger.log('Info: Phantom instance restart');
        self._cancelTtlTimeout();
        self._phantomRestart();
      });
    }
  ...
  _apply: function() {
    ...
    this._impressAttemptTimeoutId = setTimeout(
      function() {
        logger.error('Impress attempt timeout', self.attemptTimeout);
        retry();
      },
      this.attemptTimeout
    );
    ...
    function done(result) {
      ...
      if (!result.contentType || /html/i.test(result.contentType)) {
        self.htmlFilter.apply(result.content, function(err, content) {
          if (err) {
            logger.error('Error: Html filter error', err);
          }
          else {
      ...
      if (!(self._doneCounter % self.countToRestart)) {
        self._doneCounter = 0;
        self._phantomRestart();
      }
    }
    ...
    try {
      req = http.request(requestOptions, function(res) {
        ...
        if (res.statusCode != 200) {
          logger.error('Error request to impress server with http status code', res.statusCode);
          retry();
          return;
        }
        res.on('error', function(error) {
          ...
          logger.error('Impress request error', error);
          retry();
        });
        ...
        res.on('end', function() {
          ...
          if (!error && !result.ok) {
            logger.error('ERROR impress page "' + url + '":', (result.errors || []).join());
            self._performImpressReport(result);
            retry();
            return;
          }
          if (!error) {
            if (!result.contentType || /html/i.test(result.contentType)) {
              error = self._validateHtmlContentAndGetValidationError(result.content);
            }
          }

          if (error) {
            logger.error('ERROR page "' + url + '" could not be impressed. Try next attempt.', error);
            retry();
          }
          else {
            logger.log('OK page', result.httpStatusCode, '"' + url + '"', 'in time', responseTime, 'ms');
            done(result);
          }
        });
  ...
  _abortImpressAttempt: function() {
    try {
      ...
      logger.error('Could not abort impress request', e);
    }
  ...
  _validateHtmlContentAndGetValidationError: function(content) {
  ...
  _performImpressReport: function(result) {
    ...
    if (this.warnings && result.warnings && result.warnings.length > 0) {
      logger.warn('IMPRESS WARNINGS for page "' + (result.url || url) + '":\n', result.warnings.join('\n'));
    }
    if (this.notices && result.notices && result.notices.length > 0) {
      logger.info('IMPRESS NOTICES for page "' + (result.url || url) + '":\n', result.notices.join('\n'));
    }
  }
